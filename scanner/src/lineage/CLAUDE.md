...
| `validate.js` | Hand-rolled structural validator (`validateGraph`) — no new npm dependency; `dataflow-graph.schema.json` is the JSON-Schema-dialect twin, kept in parity by `test/lineage/json-schema-parity.test.js`. Checks id-prefix format (regex per entity kind) and `dataClasses`/`aiContexts` array *contents* against `classification.js`, not just array presence — closed after an initial gap let a made-up class/context value validate cleanly (see git history on `validate.js`). Does **not** yet check duplicate-id uniqueness across nodes/edges/dataElements/flows — a real gap, tracked for a follow-up task before Milestone 1 treats `validateGraph()` as a complete gate. |
