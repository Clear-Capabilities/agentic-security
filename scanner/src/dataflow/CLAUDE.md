...
- **Catalog-driven source/sink matching.** Add entries in `catalog.js`. Each entry: `kind` ∈ {source, sink, sanitizer}, plus language + framework + match shape. 200+ entries spanning Express/Flask/FastAPI/Django/Rails/PHP/Go-net-http/Gin/Echo.
  ...
  - **`match.receiver`** (optional, regex source string) constrains a sink to call sites whose receiver chain has a matching segment. Callee matching is by *bare name*, so `callee: 'write'` otherwise matches `process.stdout.write` and `fh.write` as readily as `document.write`. Set it on any DOM/receiver-specific sink.
  - **`language`** is enforced by `_languageAllowed` for `cpp` and `js` only. Other languages' entries can still fire cross-language on a same-named callee — see the deferred item on bare-callee-name sinks.
