...
## Finding provenance — `provenance/` (12 modules)

The only SUBDIRECTORY under `posture/`, because it is a pipeline rather than an
annotator: twelve small modules that together answer "which commit introduced
...
| `sca-origin.js` | which commit moved a declared dependency version into an advisory's vulnerable range |
...
| `schema.js` | the status/method/role/confidence enums, `emptyProvenance`, `redactFindingProvenance` |
...
- **One budget for the whole scan.** `engine.js` computes ONE `deadlineAt` and
  passes it to both of its `annotateGitProvenance` calls (SAST findings, then
  direct SCA deps); a caller-supplied `deadlineAt`/`perFindingBudgetMs` wins over
  the coordinator's own computation. Inside, each finding gets
