...
- **The PoC-re-check leg is similarly dormant through the MCP surface.** `verifyFixCore` accepts a `poc` param and, when given one with `poc.code` set, re-runs the proof harness against the patched files (`fix-verify.js`, the `pocLeg` block). `mcp/tools.js`'s `verify_fix` `inputSchema` has no `poc` property (`additionalProperties: false`, only `stable_id`/`files`), so every call through the MCP surface reaches this leg with `poc` undefined and it reports `{status: 'not-requested', tier: null, reason: null}` — never exercised, never counted against `ok`. The leg's result is forwarded in the tool's response (`tests`/`honesty`/`poc` were previously silently dropped from the returned object; fixed to forward all three), so the `not-requested` status is now visible to a caller instead of invisible — but visibility isn't reachability. Wiring a real `poc` through would need a schema change plus a decision about where the PoC comes from (the original finding's `f.poc`, most likely) — not attempted here since it's a design choice, not a bug fix.
...
**`verifyRunAttestation` itself has zero production callers today** (grepped `src/`, `bin/`; only `test/attestation.test.js` calls it — which is why `npm run test:lifecycle` doesn't flag it, since that guard counts test files as call sites). `computeRunAttestation` is genuinely wired (see above); the verify half is a library primitive nothing in this repo's own CLI or MCP surface currently invokes — a third party checking a shipped attestation is expected to call it from their own tooling, not from `agentic-security` itself. Distinct from `agentic-security verify-attestation`, which is CLI-wired but verifies a different artifact (`evidence-bundle.js`'s Ed25519 per-finding bundles, not this module's HMAC per-run digest) — don't conflate the two when auditing for dead code.
