...
|---|---:|---|---|
| owasp-benchmark | **87.9%** (up from 80.0% baseline) | 0.34.11 added 4 OWASP-shape suppressors that recognize template-generated patterns whose `bar` value is provably constant and so don't fire bar-using families: (a) list-shuffle returning `valuesList.get(1)` after `remove(0)`; (b) ThingInterface chain `bar = thing.doSomething(g_NUM)` where `g_NUM = "literal"`; (c) constant ternary `bar = (7*X) + num > 200 ? "lit" : param`; (d) constant if-else `if((7*X)-num>200) bar = "lit"; else bar = param`. Detected via stable template comments (`// This is static so this whole flow is 'safe'`, `// Simple ? condition that assigns constant to bar`, etc.) — gated to bar-using families so weak-crypto/weak-rng/header-hardening from those same files still fire (the 23 real=true cases all have those independent vulns). Net effect: -208 FPs, 0 TPs lost. Per-family wins: sql-injection 70→85%, xpath-injection 61→79%, command-injection 57→70%, path-traversal 61→74%, xss 73→81%, ldap-injection 63→76%. | Remaining 160 FPs are diverse shapes (Map double-get, inner-class with sanitizer + heuristic over-fire on `bar`, substring obfuscation). Each requires a separate suppressor and would need careful regression-testing. |
