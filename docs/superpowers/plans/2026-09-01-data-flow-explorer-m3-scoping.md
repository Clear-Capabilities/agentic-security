...
| Wire | **Live-API wiring for the existing three views** | Server | Medium | Replaces `frontend/`'s static-generated-module import with real API consumption; proves byte-for-byte rendering equivalence against the current static-import baseline before trusting AC-16/17/18/19's existing passing status against the new data path. Safe to proceed at the current flagship-fixture scale (14 nodes) regardless of M3-Render's own timeline — Perf's failure is a large-graph problem, not a functional one. |
