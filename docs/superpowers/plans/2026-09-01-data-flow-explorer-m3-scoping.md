...
|---|---|---|---|---|
| Perf | **Performance-gate measurement** | none | Small | A generated synthetic graph at §21's own node/edge budget; measures the current vanilla-JS renderer against it; resolves whether the React/Cytoscape/ELK migration is actually needed, with real numbers, not speculation. |
| Server | **Local server + secured read-only API** | Perf (architecture-neutral, but sequenced after so no server-side rendering assumption is baked in before the stack question settles) | Large | The P0 read-only endpoint subset (`scan`/`graph`/`nodes/:id`/`edges/:id`/`flows/:id`/`query`/`export`) plus the FULL §17.4 security list as a first-class same-sub-project requirement, with T2 (DNS rebinding)/T3 (CSRF)/T4 (path traversal)/server-side T5 (request-size/query-complexity caps) closed against the threat model doc's own pre-specified shapes. |
| Wire | **Live-API wiring for the existing three views** | Server | Medium | Replaces `frontend/`'s static-generated-module import with real API consumption; proves byte-for-byte rendering equivalence against the current static-import baseline before trusting AC-16/17/18/19's existing passing status against the new data path. |
| Inventory | **Inventory view (net new)** | Wire | Medium | The fourth required view (§7.6), built against the live API from the start — sortable tables for sources/sinks/fields/destinations/stores/AI-systems/transformations/unprotected-edges/policy-permitted-flows/manual-governance-gaps/unsupported-candidates, sharing filters/canonical IDs with the graph views per §7.6's own explicit requirement. |
...
Write M3-Perf's implementation plan first — it is small, fast, and
resolves the one architectural question every other sub-project's own
plan would otherwise have to guess at.
