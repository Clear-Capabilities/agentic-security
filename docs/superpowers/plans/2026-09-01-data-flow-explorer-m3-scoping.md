...
| Render | **Rendering fix — COMPLETE (2026-09-01), narrow SVG fix** | Perf | Medium-Large | User explicitly chose a narrow SVG fix (level-of-detail clustering + edge aggregation + real pan/zoom + viewport culling) over a full React/Cytoscape/ELK migration or a Canvas/WebGL renderer. **Real re-measurement against Perf's own exact methodology PASSES**: first-contentful-paint 148ms (vs. Perf's own "no paint after 20+s" at the same 5,000-node/10,000-edge scale), pan/zoom sustaining ~53fps against the 45fps target, both measured in real Chrome, not assumed from the math. Two real regressions found and fixed during implementation (a golden-test-breaking edge-aggregation over-scoping bug; a keyboard-focus-loss bug on rerender). Full results in `frontend/CLAUDE.md`'s own Render section. |
...
| Inventory | **Inventory view — COMPLETE (2026-09-01)** | Wire | Medium | The fourth required view (§7.6): sortable tables for all 11 named categories, sharing filters/canonical IDs with the graph views. The "table-shaped views are far less exposed to Perf's own SVG-scaling failure" assumption was re-confirmed, not just assumed, at implementation time. Full rationale in `docs/superpowers/plans/2026-09-01-data-flow-explorer-m3-inventory-scoping.md` and its companion plan. Did NOT end up needing M3-Render — current fixture/graph scale never approached §21's thresholds. |
| XSS | **T1 adversarial-fixture XSS test suite + CSP hardening** — **COMPLETE, 2026-09-01** | Wire (needs the real rendering path, not the static one, to test against) | Small | **Shipped**: the adversarial fixture + a real DOM-sweep test across all three views, proven non-vacuous via a hand-built mutation-proof AND a real, reverted mutation of `architecture-view.js` itself. CSP hardening was already covered by prior increments (Server/Wire). Disclosed, deferred: no view bounds rendered label length (a real, confirmed gap against the threat model doc's own mitigation clause, judged UX not XSS-relevant given `el()`'s own escaping). See `frontend/CLAUDE.md`'s own `test/xss-adversarial.test.js` row. |
| A11y | **Contrast + viewport reflow (AC-20/AC-21) — COMPLETE (2026-09-01)** | Wire, Inventory | Medium | Formal contrast-ratio tests found and fixed a real light-theme WCAG AA violation; keyboard-focus parity swept clean on the first pass; a real CDP measurement at all four named viewports confirmed a real bug (the inspector was unreachable via `display:none` below 1280px, contradicting §7.7's own "becomes an overlay" text) and verified its fix genuinely works, not just that the CSS looked right. Full results in `frontend/CLAUDE.md`'s own A11y section. |
| Golden | **Golden/state-matrix reference-composition tests (AC-16/17/18/19/22) — COMPLETE (2026-09-01)** | Wire, Inventory, A11y | Medium-Large | **Shipped**: golden-DOM regression tests proving AC-16/17/18/19's already-shipped reference-composition content still renders correctly against the real flagship fixture — Architecture View's 9 nodes/5 zones/dimming/raw-vs-masked verdicts, Privacy View's 3 fields across 6 lifecycle stages plus real governance-badge text, Trace View's real 4-step (not the PRD table's illustrative 5-step) trace with both rename mappings and per-alternate verdicts, and AC-16's cross-view selection/filter persistence via a real dispatched tab click. **AC-22 does NOT pass** — of the §8.4 11-state matrix, only 3 states (Error, Selected, Hovered) have real UI and real tests (`frontend/test/golden-state-matrix.test.js`); the other 8 (Loading/scanning, Partial, Truncated, Unsupported-banner, Unresolved-destination's specific dashed-edge/question-mark treatment, Zero filtered results, Error's own retry/export-diagnostics sub-action, Stale artifact) have no dedicated UI anywhere in `src/`/`styles/` today and are recorded as permanent, visible `test.todo(...)` entries (`frontend/test/golden-state-matrix-gaps.test.js`), each naming exactly what UI is missing — not silently claimed covered, not invented as placeholder UI. Full rationale in `docs/superpowers/plans/2026-09-01-data-flow-explorer-m3-golden-scoping.md`; results in `frontend/CLAUDE.md`'s own Golden section. |
| I | **Exit-gate closure (agent-closable half only) — COMPLETE (2026-09-01)** | All above | Small | Every AC-16–22 has a real, verified proof (status table in `frontend/CLAUDE.md`'s own Sub-project I section) or, for AC-22's 8 untestable states, an honest `test.todo` disclosure — never silently claimed. The usability-testing clause of the exit gate is named as permanently outside an agent's reach, with exactly what a real usability-test session would need spelled out. Milestone 3's own exit gate is therefore NOT fully satisfied (that clause, plus AC-22 itself, remain open) — this is the honest, final status, not a claim of completion. |
