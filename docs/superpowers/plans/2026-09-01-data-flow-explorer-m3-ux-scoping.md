...
|---|---|---|---|---|
| Query | **Query language parser + Focus controls + Saved views + basic text search** | Render (needs the real dimming/clustering mechanism to build on) | Large | A hand-written recursive-descent parser for §15.2's exact grammar (field:value comparisons, `AND`/`OR`, parenthesized groups, `!=`, multi-value `(a,b,c)` lists), producing a pure predicate function over `{node, edge, flow}` — reused by BOTH the query bar AND the 9 focus controls (each control is really a canned query/graph-traversal applied to the current selection). Saved views (`PCI Exposure`, `AI + Regulated Data`) as real, computed presets. Actionable parse-error reporting (§15.2's own explicit requirement) — a malformed query never silently broadens. Own dedicated scoping+plan, own SDD cycle. |
| Filters | **Expand filter-rail.js to the 9 real dimensions** | Query (shares the predicate-evaluation engine, avoids two parallel filter mechanisms) | Medium | Extends `computeFilterFacets`/`renderFilterRail` from 3 to 9 real dimensions (decision 3's own list), wired into every view the rail currently serves (Privacy, Inventory). Explicitly does NOT add UI for the 6 inert/unbacked dimensions. |
