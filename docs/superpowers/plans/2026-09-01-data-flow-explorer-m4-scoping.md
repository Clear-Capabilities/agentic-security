...
|---|---|---|---|---|
| 1 | MCP read-only tools | Small–Medium | none | Follows the exact existing pattern (`query_taint`/`explain_finding`): read-only, ID-scoped, already-hardened MCP server. No new backend capability — just new tools reading the already-built graph. Best first pick: self-contained, no blockers, low risk. |
