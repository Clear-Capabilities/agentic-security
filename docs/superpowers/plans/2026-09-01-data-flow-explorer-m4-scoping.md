...
| 3 | Self-contained HTML report | Medium **— technical question resolved, architecture decision pending, see own doc** | #2 (JSON export, COMPLETE) | Detailed investigation (`2026-09-01-data-flow-explorer-m4-html-report-scoping.md`) found the "no build step" framing above does NOT transfer to the export artifact: `frontend/`'s 21 JS files' real `import`/`export` graph only resolves correctly over `http://` (the `explore` server or a dev server). Chromium browsers refuse to resolve ES-module imports against a `file://` origin — **empirically confirmed via a real, locally-installed Chrome headless run** (`claude-in-chrome`'s own `navigate` tool refuses `file://` URLs, so the real Chrome binary was invoked directly; a minimal same-directory dynamic import genuinely failed with `Failed to fetch dynamically imported module`). A genuinely self-contained, double-clickable export needs real bundling (21 files inlined into one script), not a copy-paste — 3 architecture options named, with a recommendation (a hand-rolled minimal bundler, no new dependency) that deliberately stops short of unilaterally committing to a bundler devDependency, which this codebase's own conventions treat as a decision to re-open, not a default. Not yet planned; needs that architecture choice confirmed first. |
...
3. **Self-contained HTML report** (#3) — BLOCKED on one real open
   question (`file://` module-CORS — see own doc); medium, high reuse of already-
