...
| 4 | PNG/SVG/PDF export | Large **— technical question resolved, materially smaller than expected, see own doc** | #3 (self-contained HTML report, COMPLETE — a real, disclosed correction to this row's own original "no dependency named" framing) | Detailed investigation (`2026-09-01-data-flow-explorer-m4-image-export-scoping.md`) found this deliverable does NOT need a hand-written SVG-string serializer or a new headless-browser dependency: a real, already-installed Chrome binary's own native headless flags (`--screenshot`, `--print-to-pdf`, `--dump-dom`) produce PNG (empirically confirmed byte-identical across repeated runs — real determinism, not assumed), PDF, and (via DOM extraction) SVG output directly from #3's own already-shipped self-contained HTML report, at exactly AC-23's required 1680×945/3360×1890 dimensions. Real, disclosed remaining gaps: multi-view report capture (which view is active on screenshot), Chrome-binary discovery, and whether operators without Chrome installed can use this feature at all — a real product question, not a code gap. (SVG-element disambiguation was an initial false lead — re-checked and confirmed there is exactly one real `<svg>` in the whole page, see the sub-project's own doc.) Not yet planned; the architecture is now well-grounded. |
...
4. **PNG/SVG/PDF export** (#4) — large; technical question resolved (a
   real, already-installed Chrome binary's own headless flags, no new
   dependency — see own doc). Not yet planned; real remaining gaps
   (multi-view capture, Chrome discovery) named in that doc.
