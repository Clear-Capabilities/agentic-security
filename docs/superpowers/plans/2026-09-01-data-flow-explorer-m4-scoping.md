...
6. **Regulatory Obligation Overlay + evidence packs** (#6) — scoped;
   large, split into 6a (extension contract) / 6b (predicate engine,
   reusing `auditor-walkthrough.js`'s typed-predicate architecture) /
   6c (evidence-pack export). `governanceRefs` is NOT a real signal
   yet (see own doc) — `flow.policyVerdict` and edge protection are.
