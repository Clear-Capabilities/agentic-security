...
| 5 | CLI and Claude slash commands | Small–Medium **— scoped (2026-09-01), see own doc** | #1–#4 (all COMPLETE) | Detailed investigation (`2026-09-01-data-flow-explorer-m4-cli-slash-scoping.md`) found this is "thin wiring" but not zero-decision: no `commands/*.md` file mentions `explore`/`dataflow`/`lineage` today (zero existing slash surface for this whole PRD area), the PRD's own §7.1 names the command `visualize` while the actually-shipped verb is `explore` (ruling: keep `explore`, add a separate new `dataflow export` verb rather than retrofit), `--class PCI`-style semantic filtering has no underlying resolver anywhere (ruling: defer, ship the existing raw `{nodeIds,edgeIds}` filter shape via `--filter <path>` instead), AC-23's two pinned PNG sizes were never given a named CLI convenience (ruling: add `--size standard|2x`), and the six export/report functions split across two different failure conventions (async `{ok,reason}` for images vs. sync-throwing for JSON/CSV/HTML) that a CLI wrapper must reconcile into one consistent exit-code contract. `explore`'s own `--help`/`USAGE` entry is also currently missing — a real, pre-existing gap this sub-project will close as a drive-by fix. Not yet planned. |
...
5. **CLI and Claude slash commands** (#5) — scoped; small–medium, a new
   `dataflow export` CLI verb + `commands/dataflow.md` slash dispatcher,
   several real naming/scope decisions already ruled on in its own doc.
