...
| 3 | Self-contained HTML report | Medium **— blocked on one real open question, see own doc** | #2 (JSON export, COMPLETE) | Detailed investigation (`2026-09-01-data-flow-explorer-m4-html-report-scoping.md`) found the "no build step" framing above does NOT transfer to the export artifact: `frontend/`'s 21 JS files' real `import`/`export` graph only resolves correctly over `http://` (the `explore` server or a dev server) — Chromium browsers refuse to resolve ES-module `import` statements against a `file://` origin (well-established browser CORS behavior; could not be empirically re-confirmed in-session because `claude-in-chrome`'s own `navigate` tool refuses `file://` URLs outright). A genuinely self-contained, double-clickable export needs real bundling (21 files inlined into one script), not a copy-paste — 3 architecture options named, including a real "add a bundler devDependency?" decision this codebase's own conventions treat as consequential. Not yet planned; needs the open question resolved first. |
