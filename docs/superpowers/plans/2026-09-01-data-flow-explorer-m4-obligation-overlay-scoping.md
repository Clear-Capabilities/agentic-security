...
2. **6b — the predicate/mapping engine.** The real work: a new
   graph-fact-reading predicate type added to the SAME typed-predicate
   architecture `auditor-walkthrough.js` already implements, evaluated
   against 5 real framework catalogs (GDPR/CCPA/HIPAA/EU AI Act/NIST
   Privacy Framework — reusing the 5 that already have real catalogs;
   PCI DSS excluded per correction #4), honoring the OSCAL precedent's
   "never claim decided for what wasn't" discipline and AC-28's
   "evidence ≠ compliance" wording verbatim in any rendered output.
