...
**Update, post-implementation:** Sub-project A shipped (`scanner/src/lineage/field-identity.js`, `engine.js`, `DESIGN_INTRAPROCEDURAL.md`) after 6 rounds of adversarial re-review, each finding a genuine FR-301 violation the previous round's fix didn't cover — converging on a durable invariant ("every path component the IR supplies must be either a real property name or an explicitly-modeled unknown; every kill must be justified as a strong update on a definite location") that the final round's review confirmed is now structurally, exhaustively closed. One real, distinct, pre-existing bug was found in the process but deliberately NOT fixed in Sub-project A, since it's a differently-shaped problem with unclear blast radius into the shared taint engine: **object spread/rest (`{...user}`, `const {...rest} = user`) is silently dropped entirely by `scanner/src/ir/parser-js.js`** (confirmed pre-existing since that file's first commit, not a regression from Sub-project A) — a real identity loss (false negative), documented in `DESIGN_INTRAPROCEDURAL.md`'s object-spread bullet. **Recommendation for whichever future sub-project picks this up**: scope it as its own small unit of work (likely touching the shared parser, so needs `test:dataflow` verification alongside `test:lineage`), not folded silently into B–H's other work.
