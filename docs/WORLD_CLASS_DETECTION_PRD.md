...
| **T2.1 — comment blindness enforced engine-wide** | **Landed 2026-08-19** | §8c recorded the highest-volume instance of this class (PHP backticks, 105 findings, matched against English prose in a `//` comment) and fixed it in ONE detector. This generalises the fix and makes the property testable. Three defects, each proven with a failing test first: (1) `runFullScan` handed the RAW file to every SAST module, so comment-blindness depended on whether each author remembered `blankComments()` — 64 of 117 modules did, ~40 of the rest run regexes over raw text; (2) the engine's own `stripNoise()` handled `//` and `/* */` but not `#`, so every engine-internal regex pass leaked on Python/Ruby/shell/HCL comments; (3) `blankComments()` let a `'`/`"` string span a newline, so an ODD number of quotes on one line — which regex literals produce routinely — left the scanner stuck in string mode for the **whole rest of the file**, silently disabling comment stripping from that point on. Now: 99 dispatch sites receive a comment-blanked view (20 keep raw, each justified — the prompt-injection family, the secrets scanners, `scanTodosNearSecurity`); all 15 `stripNoise()` call sites pass a path; Ruby `=begin`/`=end` is stripped. Measured: self-scan 458 → **427** on this repo, diffed finding-by-finding — **33 removed, all 33 on comment lines, zero real findings lost**; the 2 added were previously MASKED by defect (3). Corpus 215/215, mutation 9/9, `npm test` green. |
...
| **T3.3 container/collection-element taint** | **Landed 2026-08-19 — capability added, recall UNCHANGED** | Probed ten container shapes before writing anything: JS arrays already worked (`push`/index-write/`concat`/`join` via the R4 rule, `arr.map(cb)` via the higher-order path). Three real gaps, each with a failing test first. (1) A computed write with a non-literal key lowers to `bag.*`, and `'*'` is a LITERAL property name — the lattice propagates only DOWN from a prefix, so the write was unreachable from any correctly-computed read; an unknown key could be any key, so it now widens to the container (fixed once in `_addPathAliasAware`, scoped to a trailing `.*`). (2) `Map.set`/`Set.add` joined the mutator list. (3) **The mutator rule required `callee.kind === 'member'`, a shape only Babel emits** — so container writes in Python/Ruby/PHP/Go/Java/C#/Kotlin never matched at all, and the miss was masked because a PY-SAST pattern rule caught the same sink. **`bench:layer-recall` is unchanged at 116/215** and the dataflow-shaped subset unchanged at 115/137: the corpus holds no container-shaped entries, which is why `TAINT_RECALL_80PCT_PRD.md` could close at 83.9% without Tier 5. Reported as capability pinned by `test/container-taint.test.js` (incl. 2 precision cases that must NOT fire), **not** as a recall gain. Corpus 215/215, mutation 9/9, self-scan 427 with zero new findings. |
| **Harness: the independent runner could hang indefinitely** | **Fixed 2026-08-19** | A full run wedged at entry 186/315 on `GHSA-hcm8-x79p-wx2w` (apache/camel, 649 MB — the largest tree in the population): process alive, state `S`, 0.0% CPU, no progress for six hours; killed, 129 entries never scored. **Reproduced on a stashed clean checkout, so it is pre-existing, not a regression.** `runScan` has a deep-mode walltime budget and a per-file timeout, but nothing bounded a whole-entry scan, and a block that never resolves is not an overrun any of them observe. Added a per-entry watchdog (`AGENTIC_SECURITY_BENCH_ENTRY_TIMEOUT_MS`, default 600 s) that marks the entry UNSCORED — the harness's own documented doctrine for an entry that could not be run, never a miss. Verified end-to-end against a one-entry manifest: reports the casualty by name and completes. Consequence worth stating: **every independent-population figure this project has published came from a harness that could silently wedge partway through.** |
...
> **Superseded in part, 2026-08-19.** Re-measured on 315 entries at engine
> 0.137.1: localized recall is **11/315 (3.49%)**, not the 6/315 (1.90%) below,
> localized precision **40.74%** (was 26.09%), and fix-discrimination **9/11
> (81.8%)** — up from 2/6 (33%), which clears this PRD's §6 criterion 1 target
> of ≥80% on the development population. `byLayer` now reads
> `OWNERSHIP-AUTHZ:3, LOGIC:2, FILE-UPLOAD:1, JAVA:1, ZIP-SLIP:1,
> AUTHZ-MATRIX:1, RESOURCE:1, CSHARP:1` — so **OWNERSHIP-AUTHZ (Theme 5) and
> RESOURCE (T4.2) are no longer silent**, contradicting the paragraph below.
> CONVENTION, REDIRECT-TOCTOU and CODEGEN remain at zero, so the fixture-first
> rebuild prescribed at the end of this section still applies to those three.
> The diagnosis below — rules written from this document's prose rather than
> from the vulnerable files — was correct and is retained for that reason.
