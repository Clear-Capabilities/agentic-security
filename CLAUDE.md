...
| `scanner/src/posture/provenance/` | Finding Provenance: which commit introduced each finding, resolved from Git history. 12 modules; one entry point, `annotateGitProvenance` — **never** `annotateProvenance`/`annotateFindingProvenance` (both taken by unrelated features `engine.js` already imports), and the field is `findingProvenance`, never bare `.provenance`. | see the "Finding provenance" section of `scanner/src/posture/CLAUDE.md` |
